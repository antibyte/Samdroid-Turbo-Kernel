...
//#define __TRACE_FULL_CAMERA_DRV__
	...
	static unsigned int gpf_config;
	static unsigned int gpf_data;
	static unsigned int gpf_pull;
	static unsigned int gpf_config_idle;
	static unsigned int gpf_data_idle;
	static unsigned int gpf_pull_idle;
#endif
...
/*
	case VIDIOC_S_AF:
		ctrl = (struct v4l2_control *) arg; 
		ret = cfg->cis->sensor->driver->command(cfg->cis->sensor, SENSOR_SET_AF, (void *) arg);
		break;
	
	case VIDIOC_G_AF:
		ctrl = (struct v4l2_control *) arg; 
		ret = cfg->cis->sensor->driver->command(cfg->cis->sensor, SENSOR_GET_AF, (void *) arg);
		break;
*/	
...
/*	case VIDIOC_S_SET_PREVIEW:
		ctrl = (struct v4l2_control *) arg; 
		ret = cfg->cis->sensor->driver->command(cfg->cis->sensor, SENSOR_SET_PREVIEW, (void *) arg);
		break;

	case VIDIOC_S_START_CAPTURE:
		ctrl = (struct v4l2_control *) arg; 
		ret = cfg->cis->sensor->driver->command(cfg->cis->sensor, SENSOR_START_CAPTURE, (void *) arg);
		break;

	case VIDIOC_S_JPEG_TRANSFER:
		ctrl = (struct v4l2_control *) arg; 
		ret = cfg->cis->sensor->driver->command(cfg->cis->sensor, SENSOR_JPEG_TRANSFER, (void *) arg);
		break;*/
...
#if 1	// for idle Current GPIO Setting
	__raw_writel(gpf_config, S3C64XX_GPFCON);
	__raw_writel(gpf_data, S3C64XX_GPFDAT);
	__raw_writel(gpf_pull, S3C64XX_GPFPUD);
	msleep(5);
#endif
...
// antibyte - We are already at 800 with the performance gov
...
// antibyte - dont mess arround with the frequencys
#ifdef CONFIG_CPU_FREQ
	set_dvfs_level(1);
#endif /* CONFIG_CPU_FREQ */
...
#if 1	// for idle Current GPIO Setting
	__raw_writel(gpf_config_idle, S3C64XX_GPFCON);
	__raw_writel(gpf_data_idle, S3C64XX_GPFDAT);
	__raw_writel(gpf_pull_idle, S3C64XX_GPFPUD);
#endif
...
#if 1	// for idle Current GPIO Setting
	gpf_config = __raw_readl(S3C64XX_GPFCON);
	gpf_data = __raw_readl(S3C64XX_GPFDAT);
	gpf_pull = __raw_readl(S3C64XX_GPFPUD);
	gpf_config_idle = gpf_config & (~0x03FFFFFF);
	gpf_config_idle |= 0x1555555;
	__raw_writel(gpf_config_idle, S3C64XX_GPFCON);
	gpf_data_idle = gpf_data & (~0xFFF);
	__raw_writel(gpf_data_idle, S3C64XX_GPFDAT);
	gpf_pull_idle = gpf_pull & (~0x03FFFFFF);
	gpf_pull_idle |= 0x1555555;
	__raw_writel(gpf_pull_idle, S3C64XX_GPFPUD);
#endif
