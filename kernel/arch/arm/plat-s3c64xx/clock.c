...
static const u32 s3c_cpu_clk_tab_800MHz_oc[][6] = {
	{1064*MHZ,  266 *MHZ, 532, 3, 0, 3},
	{532*MHZ,  266 *MHZ, 532, 3, 1, 3},
	{266*MHZ,  266 *MHZ, 532, 3, 3, 3},
	{133*MHZ,  266 *MHZ, 532, 3, 7, 3},
#ifdef USE_DVFS_AL1_LEVEL
	{133*MHZ,  133 *MHZ, 532, 1, 7, 7},
#endif /* USE_DVFS_AL1_LEVEL */
	{ 66*MHZ,  133 *MHZ, 532, 1, 15, 7},
};
...
#ifdef CONFIG_SPICA_OC
	s3c_cpu_clk_tab_800MHz_oc,
#else
	s3c_cpu_clk_tab_800MHz,
#endif /* CONFIG_SPICA_OC */
} ;
...
static u32 s3c_cpu_clk_tab_size(void)
{
	...
	if(S3C64XX_FREQ_TAB)
		#ifdef CONFIG_SPICA_OC
		size = ARRAY_SIZE(s3c_cpu_clk_tab_800MHz_oc);
		#else
